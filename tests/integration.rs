...
use item_write::{write_item, write_item_batch, write_items};
...
#[blitzfilter_dynamodb_test]
async fn should_create_items_when_write_new_item_batch() {
    let items = ItemModel::generate_n::<25>();

    let client = get_dynamodb_client().await;
    let write_res = write_item_batch(&items, client).await;
    assert!(write_res.is_ok());
